...
function App() {
  ...
  const addMenu = async () => {
    ...
    if (!duplicatedCheck(menuName.value)) {
  ...
  const updateMenu = async (e) => {
    ...
    if (!duplicatedCheck(updatedMenuName)) {
  ...
  const duplicatedCheck = (targetMenu) => {
    return this.menu[this.currentCategory].find(
      (menuItem) => menuItem.name === targetMenu
    );
  };
